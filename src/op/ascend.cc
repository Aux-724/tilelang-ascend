...
Stmt AscendCopy::Lower(const LowerArgs &T, arith::Analyzer *analyzer) const {
  ...
  if (src.scope() == "global" && dst.scope() == "shared.dyn") {
    ...
    if (src.scope() == "global") {
      ...
      ss << "copy_gm_to_ub<";
      ss << get_dtype(src) << ", ";
      // Preserve the physical UB row pitch for sliced destinations.  The
      // copied extent is passed separately as maskShapeN.
      ...
      ss << "copy_ub_to_gm<";
      ss << get_dtype(dst) << ", ";
      // Preserve the physical UB row pitch for sliced sources.  Using the
      // region extent here silently flattened strided UB rectangles.
