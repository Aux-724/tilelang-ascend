...
void CodeGenTileLangAscend::VisitExpr_(const CallNode *op, std::ostream &os) {
  // Parallel T.sqrt/T.rsqrt expressions are lowered to AscendC vector
  // intrinsics before codegen.  Serial expressions reach codegen as the
  // original TIR intrinsics and must use the scalar aicore overload instead.
  // A2 serial scalar arithmetic supports float32; half scalar arithmetic is
  // rejected by Bisheng and should be vectorized or explicitly widened.
  if (op->op.same_as(Op::Get("tir.sqrt"))) {
    ICHECK_EQ(op->args.size(), 1U);
    ICHECK_EQ(op->dtype.lanes(), 1);
    ICHECK(op->dtype.is_float() && op->dtype.bits() == 32)
        << "Scalar tir.sqrt only supports float32, but got "
        << op->dtype;
    // __builtin_cce_sqrtf returns NaN for zero on current A2 toolchains.
    // Preserve TIR's sqrt(0) == 0 semantics explicitly while leaving negative
    // inputs to the device intrinsic so they continue to produce NaN.
    os << "((";
    ...
    ICHECK_EQ(op->dtype.lanes(), 1);
    ICHECK(op->dtype.is_float() && op->dtype.bits() == 32)
        << "Scalar tir.rsqrt only supports float32, but got "
        << op->dtype;
    os << "(1.0f / ((";
    PrintExpr(op->args[0], os);
    os << ") == 0.0f ? 0.0f : sqrt(";
