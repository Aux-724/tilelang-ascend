...
copy_gm_to_ub(LocalTensor<T> dstTensor, GlobalTensor<T> srcTensor,
  ...
  // A compact UB row shorter than 32 bytes must not rely on DataCopyPad's
  // multi-block destination stepping, which is expressed in 32-byte units.
  // Coalesce fully contiguous short rows into one block.  Strided GM rows
  // retain the existing padded-row representation because each UB DMA address
  // must itself be 32-byte aligned.
  if constexpr (dstN * sizeof(T) < 32) {
    if (maskShapeM > 0 && maskShapeN == dstN &&
        realSrcN == maskShapeN) {
      AscendC::DataCopyPadExtParams<T> compactPadParams(false, 0, 0, padValue);
      AscendC::DataCopyExtParams contiguousCopyParams(
          1, maskShapeM * maskShapeN * sizeof(T), 0, 0, 0);
      AscendC::DataCopyPad(dstTensor, srcTensor, contiguousCopyParams,
                          compactPadParams);
      return;
    }
  }
...
copy_ub_to_gm(GlobalTensor<T> dstTensor, LocalTensor<T> srcTensor,
  ...
  // DataCopyPad advances UB source blocks in 32-byte units.  Coalesce fully
  // contiguous compact short rows into one block; leave strided destinations
  // on the existing padded-row path.
  if constexpr (srcN * sizeof(T) < 32) {
    if (maskShapeM > 0 && maskShapeN == srcN &&
        realdstN == maskShapeN) {
      AscendC::DataCopyExtParams contiguousCopyParams(
          1, maskShapeM * maskShapeN * sizeof(T), 0, 0, 0);
      AscendC::DataCopyPad(dstTensor, srcTensor, contiguousCopyParams);
